...
sspt = model_selection.SSPT(
    ...
    grace_period=100,
    params_range={
        "latent_optimizer": {"lr": {"learning_rate": (float, (0.01, 0.1))}},
        "bias_optimizer": {"lr": {"learning_rate": (float, (0.01, 0.1))}},
    },
...
for i, (x, y) in enumerate(dataset):
    baseline_y_pred = baseline.predict_one(user=x["user"], item=x["item"])
    ...
    baseline.learn_one(user=x["user"], item=x["item"], y=y)
    sspt_y_pred = sspt.predict_one(user=x["user"], item=x["item"])
    ...
    sspt.learn_one(user=x["user"], item=x["item"], y=y)
