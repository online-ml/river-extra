...
from river import anomaly, base, compose, drift, metrics
from scipy.stats import qmc

ModelWrapper = collections.namedtuple("ModelWrapper", "estimator metric")


class Hyper_Band(base.Estimator):
    ...
    def __init__(
        ...
        sampler = qmc.LatinHypercube(d=len(params_range[self.estimator[1].__class__.__name__]))
        self._sample = sampler.random(n=nr_estimators)
        self._i=0
        self._p=0
        self._counter=0
    ...
    def _prune_bandits(self):
        ...
        half = int(len(self._bandits) /2)
        if len(self._bandits)-half>1:
            self._bandits=self._bandits[:-half]
        else:
            self._bandits=self._bandits[:-(len(self._bandits)-1)]
