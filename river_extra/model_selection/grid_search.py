...
class Grid_Search(base.Estimator):
    ...
    def __init__(
        ...
        arr_lin=[]
        for param in params_range[self.estimator[1].__class__.__name__]:
            arr_lin.append(np.linspace(params_range[self.estimator[1].__class__.__name__][param][1][0],params_range[self.estimator[1].__class__.__name__][param][1][1],round(self.nr_estimators**(1./3.))))
        grids = np.meshgrid(arr_lin[0],arr_lin[1],arr_lin[2])
